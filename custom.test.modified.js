...
describe('Custom Tests', function () {
  ...
  it('[assignment] iii. see assignment doc for details', async () => {
    ...
    // Alice sends 0.06eth funds to Bob
    ...
    //assert tranfer balances
    expect(bobReceiveUtxo.amount).to.be.equal(bobSendAmount)
    const omniBridgeTBalance = await token.balanceOf(omniBridge.address)
    expect(omniBridgeTBalance).to.be.equal(0)

    //withdrawal. if this fails,use one assert
    // withdraws a part of his funds from the shielded pool
